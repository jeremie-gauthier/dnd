...
const NON_PLAYABLE_NON_INTERACTIVE_TILE_ENTITY = ['wall', 'pillar', 'tree', 'off-map'];
...
export function inferOffMapTileEntities({
  ...
  const isNonBlockingEntity = (entity: TileEntity) =>
    entity.type === 'playable-entity' || entity.kind === 'door' || entity.isBlocking === false;
  ...
  while (currentCoordExplored !== undefined) {
    addTileToExploreAt({ column: currentCoordExplored.column, row: currentCoordExplored.row + 1 });
    addTileToExploreAt({ column: currentCoordExplored.column, row: currentCoordExplored.row - 1 });
    addTileToExploreAt({ column: currentCoordExplored.column + 1, row: currentCoordExplored.row });
    addTileToExploreAt({ column: currentCoordExplored.column - 1, row: currentCoordExplored.row });

    reachableTiles.add(translateCoordToIndex({ coord: currentCoordExplored, metadata }));
...
export function createDummyTiles({ width, height }: { width: number; height: number }): Tile[] {
...
export function parseMetadata(metadataCompiled?: string): { width: number; height: number } {
  if (!metadataCompiled) {
...
export function parseStartingPositions(startingPositionsCompiled?: string): Coord[] {
  ...
  return startingPositions.map((startingPosition) => parseCoord(startingPosition));
}
...
function parseTileEntity(entityCompiled?: string): TileEntity {
  ...
  if (isNonPlayableNonInteractiveTileEntity(kind)) {
    ...
    return { type: 'playable-entity', ...parsePlayableEntityAttributes(extras) };
  } else {
