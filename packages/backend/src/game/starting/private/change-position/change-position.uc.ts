...
export class ChangePositionUseCase implements UseCase {
  ...
  public async execute({
    ...
    this.eventEmitter.emitAsync(GameEvent.GameChanged, new GameChangedPayload({ ctx, gameId }));
  }
  ...
  private assertsCanChangePosition(
    ...
    const isUserOwningTheHero = game.playableEntities[heroId]?.playedByUserId === userId;
    ...
    const isTileFree = tile.entities.every(
      (entity) => entity.type !== 'playable-entity' && entity.isBlocking === false,
    );
    if (!isTileFree) {
      throw new ForbiddenException('Requested position is blocked by an entity');
    }
  }
  ...
  private moveHeroToRequestedPosition({
    ...
    const requestedTileIdx = translateCoordToIndex({ coord: requestedPosition, metadata });
